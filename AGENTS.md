...
- **Read before you write.** Before any nontrivial change, read `docs/HANDOFF.md` (session-to-session state), `ROADMAP.md` (future milestone architecture), and `docs/superpowers/specs/2026-08-12-track-it-design.md` (the numbered decision record, D1–D12 plus amendments A1 onward). Both exist so an agent picking up mid-project doesn't have to reverse-engineer intent from a diff.
- **Write before you hand off.** After a change substantial enough to matter to whoever works here next — a new screen, a reversed decision, a new architectural rule — update `docs/HANDOFF.md`. If it reverses or fulfills a numbered decision, add the next `A<n>` amendment to the design spec instead of letting the record go stale. When cutting a release or picking up a roadmap milestone, follow `.claude/skills/version-release/SKILL.md`.
- **Sync early against a moving target, not at merge time.** If a branch touches shared UI/theme files and lives more than a day or two, periodically check `git log --oneline <target-branch>..<branch>` and diff against whichever of `android`/`web`/`gh-pages` it targets (see `CLAUDE.md` §6 — there is no `main` anymore). Don't let two independent rewrites of the same screen accumulate for weeks before the conflict surfaces.
