...
- **Comics advance like Longbox**: a Metron-matched comic's cover and issue number now follow the issue you are reading, after each advance or position edit. *Web: inert — Metron is CORS-blocked in the browser, so the lookup fails quietly and the cover stays put.*
...
### Web (`web` branch, ported 2026-09-24)
- Same feature set on the React Native Web build; confirmations use the browser's own confirm dialog (`showAlert`).
- Swipe distances follow web's larger scale: Edit on a short left swipe, Complete past 200px.
- Fixed: a mouse swipe no longer also opens the row it ended on; a directly opened track URL offers a way back to your tracks.
- Known gaps: single-issue comic search (Metron) is blocked by the browser (no CORS) — add those by hand; browser back from the Add confirm screen leaves Add (the in-app back arrow returns to the search); barcode scanning remains mobile-only.
