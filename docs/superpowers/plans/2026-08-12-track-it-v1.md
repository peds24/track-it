...
These were found while building and are already in the code. Listed here so the
plan matches what was actually shipped.

1. **Expo SDK 57, not 51+.** Scaffolding installed the current SDK, which brought
   TypeScript 6 — where `baseUrl` is an error. The `@/` alias is configured with
   `paths` alone. Package invocations in this plan are advisory.
2. **`expo-sqlite` transactions (Task 5).** The plan specified
   `withExclusiveTransactionAsync`. In SDK 57 that passes the callback a
   *separate* connection which all inner statements must use; since
   `SqlDriver.transaction` takes a zero-argument callback, statements would have
   run outside the transaction and silently lost atomicity on migrations. The
   driver uses `withTransactionAsync`.
3. **`Tabs` import path (Task 9).** SDK 57 deprecates `Tabs` from `expo-router`
   in favour of `expo-router/js-tabs` — the same component via a supported
   subpath. The tabs layout imports from `js-tabs`.
4. **Async testing APIs (Task 10).** In `@testing-library/react-native` v14,
   `render()` and `fireEvent.press()` are async and must be awaited from `async`
   test callbacks. This plan's component test code is written synchronously and
   fails with "render function has not been called" as given.
5. **`lastAdvancedAt` (Task 10).** The plan implemented D12's "ordered by most
   recently advanced" nowhere — `listTracks` sorted every shelf by date added.
   `TrackSummary` now carries `lastAdvancedAt`, derived at read time as the
   maximum of `finishedAt`/`startedAt` across a track's children, with no schema
   change. The Currently shelf sorts by it (nulls last, falling back to date
   added); Backlog and Done keep date-added order per D9.
6. **`nextEntryId` for finished standalone tracks (Task 7).** See the amendment
   note below.
